...
namespace gplat
{
		...
		: TcpSocketType, IDisposable
	{
		...
		// acceptor / connector 용도 체크, acceptor에서 생성하는 소켓인 경우
		...
		public SyncState<sock_recv_state_e> m_recv_state = new SyncState<sock_recv_state_e>(sock_recv_state_e.not_receiving); //.net6 stackoverflow

		gplat.LogicTimer m_heartbeat_timer = new gplat.LogicTimer();
		protected gplat.LogicTimer m_connect_timer = new gplat.LogicTimer();

		//heartbeat
		//전역 기본값 : 전체 정책을 변경하는 경우 사용
		...
		public bool m_header_toucherble = false; //헤더 정보 보정 요부 for client session
		...
		public void Dispose()
		{
			...
			m_message_notifier.clearMessageQueues();//큐내용 모두제거 기능 추가
				...
				if (gplat_result.ok())
				{
					while (true)
					{
						...
						if (gplat_result.fail())
						{
							...
							{
								//받은 쪽에 invalid 패킷이 수신되었음을 내부에 알리는 메시지 정의
						...
						touchHeader(packet);
						relayToMessage(packet); //릴레이 패킷 보정
		...
		void relayToMessage(GenPacket in_packet)
		{
			...
			//릴레이 패킷을 받은 경우 원래 패킷으로 복원, 채널을 통과하지 않고 전송하는 경우 보완
		...
		protected bool sendInternal(GenPacket in_sending_packet, bool in_sync_send = false)
		{
			...
			{
				return processOfflineSend(in_sending_packet); //접속이 안된 상태라면 무조건 접속을 수행 후 전송하도록 변경
			}
		...
		{
			m_timer_thread_active = false; // 타이머 쓰레드를 죽임...
